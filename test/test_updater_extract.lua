...
local function createTestZip(zip_path)
    ...
    if okArch and Archiver and Archiver.Writer then
        ...
        if ok then
            ...
            w:addFileFromMemory(TOP .. "/docs/NOTES.txt", "docs should be excluded\n")
    ...
    write(src .. "/" .. TOP .. "/docs/NOTES.txt", "docs should be excluded\n")
...
local tests = {
    ...
    test("is_excluded_with: patterns from .releaseignore filter correctly", function()
        ...
        -- docs/ is source-only and excluded as a directory (not just via *.md)
        assert.isTrue(is_excluded_with("docs/NOTES.txt", pats))
        assert.isTrue(is_excluded_with("docs/ARCHITECTURE.md", pats))
    ...
    test("entity extraction: filtered files respected, kept files extracted", function()
        ...
        assert.isFalse(pathExists(inside("docs/NOTES.txt")), "docs/NOTES.txt excluded")
