...
Optional tuning variables (defaults shown):
    ...
    RANDOM_SESSION_ID=    # stable random id sent as x-opencode-session only when API_ENDPOINT contains "opencode.ai" (default: one id generated per make run; random per process for direct script runs)
