...
- **Provider menu paths**: `main.lua` uses fixed TouchMenu paths `4.1.8.1` for the Reader menu and `3.1.6.1` for the FileManager menu to highlight `Provider API` from Provider Settings (tab -> AI Assistant -> Settings -> Provider API). `Provider API` is no longer a top-level AI Assistant menu item — it lives as the first item of the `Settings` submenu (built by `SettingsDialog.genMenuSettings`), shared via `Assistant:getAddProviderMenuItem()` so the preset/custom submenu is defined once. If changing the menu tab order, Assistant submenu order, the `Settings` item order, or anything before `Settings`/`Provider API`, update both paths and verify both layouts. The shortcut is intentionally hidden on non-touch devices.
